...
export function PipelineCard({ data }: { data: PipelineCardData }) {
  ...
  return (
    <article className="pipe-card" data-band={band} aria-busy={pending}>
      <div className="pipe-card-rail" aria-hidden="true" />
      <div className="pipe-card-body">
        <div className="pipe-card-top">
          <span className="pipe-card-company">{data.companyName}</span>
          {data.fitScore != null && (
            <span className="pipe-card-score mono" data-band={band}>{Math.round(data.fitScore)}%</span>
          )}
        </div>
